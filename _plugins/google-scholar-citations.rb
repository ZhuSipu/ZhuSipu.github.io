...
module Helpers
  def self.number_to_human(number)
    ...
    return format("%.1fK", value / 1_000.0).sub(".0K", "K") if value < 1_000_000
    return format("%.1fM", value / 1_000_000.0).sub(".0M", "M") if value < 1_000_000_000

    format("%.1fB", value / 1_000_000_000.0).sub(".0B", "B")
  end
end
